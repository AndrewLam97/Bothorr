...
class HoningCalculator:
    ...
    def calculate_attempts_from_artisans(self, artisansEnergy, targetLvl, gearType, honingTier) -> int:
        ...
        totalBaseHoning = self.data["t3"][tierIlvl][gearType][str(targetLvl)]["baseSuccess"] + data["t3"][tierIlvl][gearType][str(targetLvl)]["strongHoldBuff"] + data["t3"][tierIlvl][gearType][str(targetLvl)]["globalBuff"]
